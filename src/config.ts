// config.ts: All magic numbers and game constants — single source of truth

import type { IWaveConfig } from './types/game.ts';
...
interface EnemyConfig {
  BASE_SPEED: number;
  SHOOT_INTERVAL_MIN: number;
  SHOOT_INTERVAL_MAX: number;
}
...
interface GameplayConfig {
  EDGE_RIGHT: number;
  EDGE_LEFT: number;
  INVADER_FLOOR_Y: number;
  SHOCKWAVE_POOL_SIZE: number;
}
